...
int main() {
    ...
    // Measured fitting framework: exact data must score zero; offsets must be visible.
    {
    ...
    // BD-2 family topology: robust invariants are finite output, PDC latency and
    // control-dependent waveform changes rather than a forced monotonic THD rule.
    {
    ...
    // Two-transistor fuzz: bias/starve must be interactive and remain stable.
    {
    ...
    // Generic amp topology verifies the full reusable chain can run as one graph node.
    {
