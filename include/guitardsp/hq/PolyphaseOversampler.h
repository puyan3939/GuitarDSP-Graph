...
public:
    void prepare(int channels, int maxBlockSize, int factor, int taps) {
        ...
        upHistory_.assign(static_cast<std::size_t>(channels_), std::vector<float>(static_cast<std::size_t>(historySize), 0.0f));
        upWrite_.assign(static_cast<std::size_t>(channels_), 0);
        downHistory_.assign(static_cast<std::size_t>(channels_), std::vector<float>(static_cast<std::size_t>(taps_), 0.0f));
    ...
    int latencySamples() const noexcept { return factor_ <= 1 ? 0 : (taps_ - 1) / (2 * factor_); }
        ...
        for (int ch = 0; ch < channels; ++ch) {
            ...
            for (int n = 0; n < numSamples; ++n) {
                ...
                for (int phase = 0; phase < factor_; ++phase) {
                    ...
                    for (float c : coeffs) {
                        acc += static_cast<double>(c) * static_cast<double>(history[static_cast<std::size_t>(r)]);
                        if (--r < 0) r = static_cast<int>(history.size()) - 1;
                    }
                    hi[n * factor_ + phase] = static_cast<float>(acc * static_cast<double>(factor_));
                }
            ...
            for (int i = 0; i < highSamples; ++i) {
                ...
                if ((i % factor_) == factor_ - 1) {
                    double acc = 0.0;
                    int r = dw;
                    for (int k = 0; k < taps_; ++k) {
                        acc += static_cast<double>(kernel_[static_cast<std::size_t>(k)]) * static_cast<double>(down[static_cast<std::size_t>(r)]);
                        if (--r < 0) r = taps_ - 1;
                    }
...
private:
    void designKernel() {
        ...
        for (int i = 0; i < taps_; ++i) {
            ...
            const double sinc = m == 0 ? 2.0 * cutoff
                : std::sin(2.0 * std::numbers::pi * cutoff * static_cast<double>(m)) / (std::numbers::pi * static_cast<double>(m));
        ...
        if (std::abs(sum) > 1.0e-12) for (auto& c : kernel_) c = static_cast<float>(static_cast<double>(c) / sum);
    }
    ...
    void buildPhases() {
        ...
        for (int phase = 0; phase < factor_; ++phase) {
            ...
            for (int k = phase; k < taps_; k += factor_) p.push_back(kernel_[static_cast<std::size_t>(k)]);
        }
    }
