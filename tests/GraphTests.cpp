...
{
public:
    explicit TestNode(std::size_t latency = 0) : latency(latency) {}

    std::string_view typeName() const noexcept override { return "TestNode"; }
    void prepare(double, std::size_t) override {}
    void reset() noexcept override {}
    void process(const ProcessContext&, const AudioBlock&, AudioBlock&) noexcept override {}
    std::size_t latencySamples() const noexcept override { return latency; }
...
int main()
{
...

    {
        ...
        graph.connect(input, fast);
        graph.connect(input, slow);
        graph.connect(fast, merge);
        graph.connect(slow, merge);
