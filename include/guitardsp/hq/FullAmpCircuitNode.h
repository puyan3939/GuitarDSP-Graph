...
// Graph node backed by the component-level full-amp MNA circuit
// (guitardsp::circuit::FullAmpCircuit): PreampCircuit's 12AX7 gain stage +
// Bass/Treble tone stack cascaded into PowerAmpCircuit's EL34 power stage,
// following the same pattern as PreampCircuitNode/PowerAmpCircuitNode: one
...
//
// Only the preamp's Drive/Bass/Treble are exposed as controls, matching a
// real amp's front panel (the power stage has no user control of its own,
// same as PowerAmpCircuit/PowerAmpCircuitNode). Raising Drive doesn't just
// distort the preamp stage harder -- because the two circuits are cascaded
// sample-by-sample (see FullAmpCircuit), a hotter preamp output also drives
// the power stage's grid harder, so the power stage's own distortion rises
// together with the preamp's, the same natural interaction a real full-amp
// signal chain has.
class FullAmpCircuitNode final : public graph::AudioNode {
public:
    ...
    std::string_view typeName() const noexcept override { return "Full Amp Circuit"; }
    ...
    const circuit::FullAmpCircuit* circuitForChannel(std::size_t channel) const noexcept {
        return channel < circuits_.size() ? &circuits_[channel] : nullptr;
    }

private:
    std::vector<circuit::FullAmpCircuit> circuits_;
